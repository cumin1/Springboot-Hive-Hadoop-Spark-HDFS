...
for i in [1,3,5,7,9,11]:    #基频加五个谐波组成的信号
...
#切比雪夫带阻波波器，cutoff ：设定两个截止频率,在两个频率之间的谐波滤掉，order：设定滤波器阶数，阶数越高波纹越陡
def cheby1_bandstop_filter(data,N,rp,fs, cutoff1,cutoff2):
    ...
    b, a = signal.cheby1(N,rp,Wn=normal_f,btype='bandstop',analog=False)
    ...
    plt.axvline(cutoff1, color='green') # cutoff frequency
    plt.axvline(cutoff2, color='green') # cutoff frequency
    ...
    # 保存图像到文件
    image_filename = "./tmp_img/cybandstop_" + str(cutoff) + "_" + str(N) + "_" + str(rp) + ".png"
...
if __name__ == "__main__":
    ...
    cheby1_bandstop_filter(data,N,rp,fs, cutoff,5.0)
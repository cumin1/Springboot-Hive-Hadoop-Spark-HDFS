...
async function fetchImages() {
    try {
        ...
        txts.length = 0; // 清空 images 数组
        txts.push(...data.data); // 将新数据追加到images数组
        renderFileList(txts); // 更新文件列表
    } catch (error) {
...
// 删除文件的函数
async function handleDelete(txt) {
    ...
    try {
        const response = await fetch(`http://localhost:8080/hdfs/file/deleteTxt/${txt}`, {
